...
class LimpiezaConcatAdapter(private val serviceAdapter: ServiceAdapter): RecyclerView.Adapter<BaseConcatHolder<*>>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseConcatHolder<*> {
        val itemBinding = LimpiezaRowBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    private inner class ConcatViewHolder(val binding: LimpiezaRowBinding) : BaseConcatHolder<ServiceAdapter>(binding.root) {
        override fun bind(adapter: ServiceAdapter) {
            binding.rvLimpieza.adapter = adapter
        }
    }
}